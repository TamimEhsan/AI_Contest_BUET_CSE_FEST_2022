...
int fireInTheHole(int id){
    ...
    while(grid[xx][yy+k]!=WALL){
        ...
        for(int i=0;i<10;i++){
            ...
            if( enemy.bots[i].curr_pos == boo and enemy.bots[i].isVisible ) {
                enemyCount++; cerr<<"x";
                if( enemy.flag_carrier == i ) enemyInVisual = true;
    ...
    if( enemy.flag_carrier!=-1 and enemyInVisual  and !hero.bots[id].curr_pos.inVisual( getNextPossiblePosOfEnemySeeker() ) ) enemyCount--;
    return enemyCount and !heroCount;  // enemyCount>heroCount;
}
...
int main(){
    ...
    while (++timer) {
        ...
        if( enemy.flag_carrier !=-1 or timer>40 or ( enemy.prev_flag_carrier!=-1 and enemy.bots[enemy.prev_flag_carrier].isVisible ) ){
            int mn_dis2 = INF;
            for(int i=0;i<10;i++){
                if( hero.bots[i].hp>0 and hero.bots[i].timeout == 0  ){
